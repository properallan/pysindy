...
from sklearn.pipeline import _final_estimator_has, available_if
class Pipeline_predict(Pipeline):
    def __init__(self, *args, **kwargs):
        super(Pipeline_predict, self).__init__(*args, **kwargs)
    ...
    def predict(self, X, **predict_params):
        ...
        **predict_params : dict of string -> object
        ...
        for _, name, transform in self._iter(with_final=False):
            Xt = transform.transform_predict(Xt)
...
class SINDy(BaseEstimator):
    ...
    def fit(
        ...
        if isinstance(self.feature_library, pysindy.feature_library.WeakPDELibrary):
            self.feature_library_prediction = copy.deepcopy(self.feature_library)
            self.feature_library_prediction.transform = self.feature_library.transform_weak_prediction
            steps_predict = [
                ("features", self.feature_library_prediction),
                ("shaping", SampleConcatter()),
                ("model", optimizer),
            ]
            self.model_predict = Pipeline(steps_predict)
    ...
    def predict(self, x, u=None, multiple_trajectories=False):
        ...
        if u is not None:
            ...
            x = [np.concatenate((xi, ui), axis=xi.ax_coord) for xi, ui in zip(x, u)]
        if isinstance(self.feature_library, pysindy.WeakPDELibrary):
            result = [self.model_predict.predict([xi]) for xi in x]
            result = [
